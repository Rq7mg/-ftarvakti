...
def create_progress_bar(percent):
    ...
    # Eğer yüzde 100 ise barı tam göster
    if percent == 100:
        bar = "▬" * 10
...
async def engine(update: Update, context: ContextTypes.DEFAULT_TYPE, mode):
    ...
    try:
        # İmsak ve İftar vakitlerini alalım (İlerleme çubuğu hesabı için ikisi de lazım)
        imsak_vakti_str = LOCAL_CACHE[city_key]["imsak"][r_day-1]
        iftar_vakti_str = LOCAL_CACHE[city_key]["iftar"][r_day-1]
        
        # Günün imsak ve iftar datetime objelerini oluşturalım
        ...
        # GÜNÜN İLERLEMESİ HESAPLAMASI
        progress_percent = 0
        
        if mode == "iftar":
            # İftar için: İmsaktan şu ana kadar geçen sürenin, toplam oruç süresine oranı
            if now > imsak_dt and now < iftar_dt:
                ...
                progress_percent = 100 # İftar vakti geçtiyse veya tam o ansa
            elif now <= imsak_dt:
                progress_percent = 0 # Henüz imsak olmadıysa
                
        else: # mod sahur ise
             # Sahur için: Önceki iftardan bu imsaka kadar geçen süre hesaplanabilir
             # Geri sayım mantığına uygun olarak oruçsuz sürenin ilerlemesini hesaplayalım
             
             # Önceki günün iftar vakti (Eğer Ramazan'ın 1. günü değilse)
             if r_day > 1:
                  ...
                  # 1. günse önceki akşam saat 18:00 baz alınabilir (örnek olarak)
                  onceki_iftar_dt = (now - timedelta(days=1)).replace(hour=18, minute=0, second=0, microsecond=0)
             
             # Eğer imsak vaktini geçtiysek ve bir sonraki günün sahurunu bekliyorsak
             if now > imsak_dt:
                  sonraki_imsak_str = LOCAL_CACHE[city_key]["imsak"][r_day] if r_day < 30 else "05:00" # Basit bir fallback
                  ...
                  # İmsak vaktinden önceysek (gece saatleri)
